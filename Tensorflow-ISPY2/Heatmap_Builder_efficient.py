...
for drug in drugs:
    full = pd.read_csv('DrugData/' + drug + '/' + drug + '1.0_full.csv')['GENE'][:30]
    t20 = pd.read_csv('DrugData/' + drug + '/' + drug + '1.0_top_20.csv')['GENE']
...
drug_leg = plt.legend(handles, drug_lut, title='Drug',
                      bbox_to_anchor=(0, 0.4), bbox_transform=plt.gcf().transFigure, loc='lower left', handleheight=2,
                      handlelength=7,
...
bp_leg = plt.legend(handles, bp_lut, title='BP-subtype',
                    bbox_to_anchor=(0, 0.28), bbox_transform=plt.gcf().transFigure, loc='lower left', handleheight=2,
                    handlelength=7,
                    fontsize=7, title_fontsize='xx-large')
...
sns_plot.savefig("DrugData/heatmap_20.pdf")

print('creating heatmap')
sns.set_context("paper", font_scale=1.3)
sns_plot = sns.clustermap(data_full[numerical_cols_full], xticklabels=True, yticklabels=False,
...
response_leg = plt.legend(handles, response_lut, title='Response',
                          ...
                          fontsize=7, title_fontsize='xx-large')
plt.gca().add_artist(response_leg)
sns_plot.savefig("DrugData/heatmap_full.pdf")
